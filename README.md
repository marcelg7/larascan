...
- **Composer install** — `composer require --dev mjgapp/laravel-audit`, use via `vendor/bin/laravel-audit`
- **Baseline file** — `--update-baseline` snapshots current findings to `.laravel-audit/baseline.json`; subsequent scans filter them. Adopt on legacy codebases without every PR failing.
- **Config file** — `.laravel-audit/config.json` sets profile, excluded rules, excluded paths, webhooks, brand. CLI overrides.
...
- **Custom rule plugins** — drop `userRule_*` functions into `<repo>/.laravel-audit/rules/`
...
- **Trend tracking** — `.laravel-audit/history/` + score delta
...
2. Move the `laravel-audit` folder into your Claude Code skills directory:
...
Claude will scan the repo, print a summary in chat, and drop a standalone HTML report at `<repo>/laravel-audit-report.html`. Open it in any browser and `Cmd+P → Save as PDF` to archive or send to a client.

You can also run the scanner directly without Claude:

```bash
# Scan only
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --pretty

# Use a named profile
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --profile=pre-launch
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --profile=rescue
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --profile=monthly

# Preview what auto-fix would change
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --fix --dry-run

# Apply safe fixes (commit or stash first — aborts on dirty working tree)
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --fix

# Save summary to .laravel-audit/history/ for trend tracking
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --save-history

# Walk findings one at a time (requires TTY)
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --interactive

# Send the summary to Slack / Discord / a generic webhook
php ~/.claude/skills/laravel-audit/scripts/audit.php /path/to/your/laravel-app --webhook=https://hooks.slack.com/...

# Compare two reports
php ~/.claude/skills/laravel-audit/scripts/diff.php last-month.json this-month.json
```
...
Copy `templates/github-workflows/laravel-audit.yml` into your repo at `.github/workflows/laravel-audit.yml`, and vendor the two scanner scripts into `.laravel-audit/scripts/` in the same repo. The workflow runs on every pull request, posts findings as a sticky bot comment, and fails the build if the grade is F.
...
The report footer will show your name, link, and optional call-to-action on the right side. Leave any field blank to omit it. Global location also works: `~/.config/laravel-audit/config.json`. Per-invocation override: `LARAVEL_AUDIT_CONFIG=/path/to/config.json`.
